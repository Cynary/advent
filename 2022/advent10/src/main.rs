...
fn main() {
    let (_, _, strength) = INPUT
        .lines()
        .map(|ln : &str| if ln.starts_with("noop") { vec![0] } else { vec![0, ln.split(' ').collect::<Vec<&str>>()[1].parse::<isize>().unwrap()] })
        .fold((0, 1, 0), |(mut cycle, mut x, mut strength), b| {
            for i in b {
                if (cycle - 19)%40 == 0
                {
                    strength += (cycle+1) * x;
                ...
                print!("{}", if (cycle%40 - x as isize).abs() <= 1 { '#' } else { '.' });
    ...
    // let mut knots : [(isize, isize); N_KNOTS] = [(0,0); N_KNOTS];

    // let mut visited = HashSet::new();
    // visited.insert(knots[knots.len()-1]);

    // for line in INPUT.trim().split('\n') {
    //     let mut inputs = line.trim().split(' ');
    //     let direction = inputs.next().unwrap();
    //     let count = inputs.next().unwrap().parse::<isize>().unwrap();

    //     let delta : (isize, isize);
    //     match direction {
    //         "U" => delta = (-1, 0),
    //         "D" => delta = (1, 0),
    //         "R" => delta = (0, 1),
    ...
    //             let (x, y) = knots[i-1];
    //             let (tx, ty) = knots[i];
    //             if (tx - x).abs() > 1 || (ty - y).abs() > 1 {
    //                 knots[i] = (tx + (x-tx).clamp(-1, 1), ty + (y-ty).clamp(-1, 1));
    //             } else {
    //                 break;
    //             }
    //         }
